...
function ENT:Use( activator, caller )
	...
	if ( activator:IsPlayer() ) then
		...
		local ammo = activator:GetAmmoCount( "buckshot" ) 

		if (ammo) < 48 then
		
			ammo = ammo + self.Value
			if ammo > 48 then ammo = 48 end
			activator:SetAmmo( ammo, "buckshot" ) 
			activator:ChatPrint("Added Shotgun Ammo ( x "..self.Value.." )")				
			...
			activator:ChatPrint("Shotgun ammo is full")
