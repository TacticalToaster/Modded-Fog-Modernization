...
function ENT:Initialize()	
	...
	self:SetNWInt( 'maxresourceamount', 15 )
...
function ENT:Use( activator, caller )
	...
	if ( activator:IsPlayer() ) then
		...
		if IsValid(activator) and activator:Alive() then
			...
			if (amount > 0 ) then
				activator:ChatPrint("You gained Circuits (x"..amount..").")
				self:SetNWInt("resourceamount", 0)
				local circuits = activator:GetNWInt("plyCircuits")
				local maxcircuits = activator:GetNWInt("plyMaxCircuits")
				circuits = circuits + amount
				if (circuits > maxcircuits) then circuits = maxcircuits end
				activator:SetNWInt("plyCircuits", circuits)
			else
