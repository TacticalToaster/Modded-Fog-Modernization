...
function ENT:Use( activator, caller )
	...
	if ( activator:IsPlayer() ) then
		...
		local ammo = activator:GetAmmoCount( "357" ) 

		if (ammo) < 12 then
		
			ammo = ammo + self.Value
			if ammo > 12 then ammo = 12 end
			activator:SetAmmo( ammo, "357" ) 
			activator:ChatPrint("Added Magnum Ammo ( x "..self.Value.." )")			
			...
			activator:ChatPrint("Magnum ammo is full")
