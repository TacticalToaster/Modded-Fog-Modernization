...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "npc_strider_enforcer" )
...
function ENT:Initialize()
	self.npc = ents.Create( "npc_strider" )
...
function ENT:CheckForHatch()
	if self.hatchTime <= CurTime() then
		//if IsValid(self.npc:GetEnemy()) then
			self:Hatch(self.npc:GetPos() + Vector(math.Rand(-1, 1), math.Rand(-1, 1), 0) * 300)
		//end
	end
end
...
function ENT:Hatch(pos)
	...
	//ent:SetModelScale(.6)
	ent:SetPos(self.npc:GetPos() + Vector(0, 0, -100))
	ent:Spawn()
	ent:SetHealth(60)
	ent:SetName("Scanner Enforcer")
