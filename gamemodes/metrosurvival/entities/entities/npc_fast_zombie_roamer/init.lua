...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "npc_fast_zombie_roamer" )
...
function ENT:Initialize()
	self.npc = ents.Create( "npc_fastzombie" )
	...
	self.npc:Spawn()
	self.npc:Activate()
	self.npc:SetHealth(180)
	self.npc:SetModelScale(1.2)
	//self.npc:SetSkin(3)
	self.npc:SetName("Fast Zombie Roamer")
	//self.npc:SetPlaybackRate(2)
	self:SetParent(self.npc)
 end

function ENT:Think()
	if IsValid(self.npc) then
		self.npc:SetPlaybackRate(1.7)
	end
